...
* `schedule(priority: Boolean, beforeDebounce: Function = ()=>{}, debounce: Function = ()=>{})` If `limitReached()` is false, `do(beforeDebounce, debounce)` is called immediately. Else, an object containing the passed functions is created and added to the start of `queue` (when `priority` is `true`) or end of `queue` (when `priority` is `false`).
* `doScheduled(max: Number = Infinity)` Calls `forceScheduled(...)` with a count that doesn't go over the throttle limits. Useful for custom scheduling logic.
